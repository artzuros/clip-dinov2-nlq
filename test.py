...
def load_faiss_index():
    if os.path.exists(DIR_PATH + "/" + "index_clip.index") and os.path.exists(
        DIR_PATH + "/" + "index_dino.index"
    ):
...
if st.button("Generate FAISS Index"):
    index_clip = faiss.IndexFlatL2(768)
...
if "index_clip" in st.session_state and "index_dino" in st.session_state:
    ...
    if query:
        ...
        D_clip_text, I_clip_text = search_index_text(query, index_clip, length)

        # Slider to control the number of images to display
        max_images_to_show = min(len(I_clip_text[0]), len(images))
        total_images_to_show = st.slider("Total images to show", 20, max_images_to_show, 20, 20)
    ...
    if "selected_images" in st.session_state and st.session_state.selected_images:
        ...
        if st.button("Process Selected Images with DINO"):
            ...
            for selected_image in st.session_state.selected_images:
                ...
                D_dino, I_dino = search_index_image(img, index_dino, length)
            ...
            cols = st.columns(5)
            for idx, (selected_image, similar_image, distance) in enumerate(
                st.session_state.dino_search_results
            ):
                col = cols[idx % 5]
                with col:
        ...
        if st.session_state.get("dino_search_export", False):
            if st.button("Export DINO Results to CSV"):
                ...
                df = pd.DataFrame(
                    data, columns=["Query", "Selected Image", "Similar Image", "Distance"]
                )
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
